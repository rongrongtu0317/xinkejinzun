  ...
  return (
    <>
      ...
      <div className="relative h-[60vh] min-h-96 bg-charcoal-800 overflow-hidden">
        {/* 详情页大图（在 src/data/products.ts 的 heroImage 字段维护） */}
        <Image
          ...
          <div className="max-w-7xl mx-auto px-6 lg:px-12 w-full">
            ...
            >
              ...
              <nav className="flex items-center gap-2 text-[10px] text-charcoal-300 tracking-widest uppercase mb-6">
                <Link href="/" className="hover:text-gold-500 transition-colors">{t.productDetail.breadcrumbHome}</Link>
                <span>/</span>
                <Link href="/products" className="hover:text-gold-500 transition-colors">{t.productDetail.breadcrumbProducts}</Link>
                <span>/</span>
                <span className="text-gold-500">{name}</span>
              </nav>

              <div className="flex flex-wrap gap-2 mb-5">
                {pick(product.colors, lang).slice(0, 4).map((c) => (
              ...
              <p className="text-warm-400 text-sm max-w-xl">{pick(product.tagline, lang)}</p>
            </motion.div>
          </div>
        </div>
      </div>
      ...
      <section className="py-20 bg-charcoal-900">
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
            {/* 左侧产品描述 */}
            <div>
              <span className="section-label">{t.productDetail.introLabel}</span>
              <div className="w-10 h-px bg-gold-500 my-4 opacity-70" />
              <h2 className="text-warm-100 font-light text-2xl mb-6 leading-snug">{t.productDetail.sellingPoints}</h2>
              <p className="text-warm-400 text-sm leading-loose mb-8">{pick(product.description, lang)}</p>

              {/* 咨询按钮 */}
              <Link href="/contact" className="btn-gold text-sm inline-flex">
                {t.productDetail.contactQuote}
              </Link>
            </div>

            {/* 右侧特性列表 */}
            <div className="flex flex-col gap-4">
              {pick(product.features, lang).map((f, i) => (
                <motion.div
                  key={i}
      ...
      <section className="py-16 bg-charcoal-800">
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <span className="section-label">{t.productDetail.applicableLabel}</span>
          <div className="w-10 h-px bg-gold-500 my-4 opacity-70" />
          <div className="flex flex-wrap gap-3 mt-4">
            {pick(product.applications, lang).map((a) => (
