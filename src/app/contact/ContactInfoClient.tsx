'use client'

import { motion } from 'framer-motion'
import { useLanguage } from '@/lib/i18n/LanguageProvider'

// 图标（结构性，与语言无关）
const icons = [
  (
    <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
      <path d="M3 3h12a1 1 0 011 1v10a1 1 0 01-1 1H3a1 1 0 01-1-1V4a1 1 0 011-1z" stroke="#3aa45e" strokeWidth="1.2"/>
      <path d="M2 4l7 6 7-6" stroke="#3aa45e" strokeWidth="1.2" strokeLinecap="round"/>
    </svg>
  ),
  (
    <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
      <path d="M3.5 2h4l1.5 3.5-2 1.2c.8 1.8 2.2 3.2 4 4l1.2-2L16 10v4c0 .8-.7 1.4-1.5 1.3C5.2 14.3 2 8 2 3.5 2 2.7 2.7 2 3.5 2z" stroke="#3aa45e" strokeWidth="1.2" strokeLinejoin="round"/>
    </svg>
  ),
  (
    <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
      <circle cx="9" cy="8" r="3" stroke="#3aa45e" strokeWidth="1.2"/>
      <path d="M9 1C5.1 1 2 4.1 2 8c0 5.3 7 10 7 10s7-4.7 7-10c0-3.9-3.1-7-7-7z" stroke="#3aa45e" strokeWidth="1.2"/>
    </svg>
  ),
  (
    <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
      <rect x="2" y="2" width="14" height="14" rx="2" stroke="#3aa45e" strokeWidth="1.2"/>
      <path d="M6 6h2v2H6zM10 6h2v2h-2zM6 10h2v2H6zM10 10h2v2h-2z" fill="#3aa45e" opacity="0.6"/>
    </svg>
  ),
]

export default function ContactInfoClient() {
  const { t } = useLanguage()
  const c = t.contactPage

  const contactItems = [
    { icon: icons[0], label: c.itemEmail,   value: 'rongrongtu0202@gmail.com', note: '' },
    { icon: icons[1], label: c.itemPhone,   value: '19322277172',             note: '' },
    { icon: icons[2], label: c.itemAddress, value: c.addressValue,            note: '' },
    { icon: icons[3], label: c.itemWechat,  value: '+86 19322277172',         note: c.wechatNote },
  ]

  return (
    <div>
      <div className="mb-8">
        <span className="section-label">{c.infoLabel}</span>
        <div className="w-10 h-px bg-gold-500 my-4 opacity-70" />
        <h2 className="text-warm-100 font-light text-xl">{c.infoTitle}</h2>
      </div>

      <p className="text-warm-500 text-sm leading-relaxed mb-10">
        {c.infoIntro}
      </p>

      <div className="space-y-6">
        {contactItems.map((item, i) => (
          <motion.div
            key={item.label}
            initial={{ opacity: 0, x: -16 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ delay: i * 0.1, duration: 0.5 }}
            className="flex items-start gap-4 py-5 border-b border-charcoal-700"
          >
            <div className="flex-shrink-0 mt-0.5">{item.icon}</div>
            <div>
              <p className="text-[10px] text-charcoal-300 tracking-widest uppercase mb-1">{item.label}</p>
              <p className="text-warm-200 text-sm">{item.value}</p>
              {item.note && <p className="text-charcoal-400 text-[10px] mt-1">{item.note}</p>}
            </div>
          </motion.div>
        ))}
      </div>

      {/* 工作时间 */}
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ delay: 0.5, duration: 0.5 }}
        className="mt-10 p-5 bg-charcoal-800 border border-charcoal-700"
      >
        <h4 className="text-warm-200 text-sm font-medium mb-3">{c.workHours}</h4>
        <div className="space-y-2 text-xs text-charcoal-200">
          <div className="flex justify-between">
            <span>{c.weekdays}</span>
            <span>09:00 - 18:00</span>
          </div>
          <div className="flex justify-between">
            <span>{c.saturday}</span>
            <span>09:00 - 12:00</span>
          </div>
          <div className="flex justify-between text-charcoal-400">
            <span>{c.sunHoliday}</span>
            <span>{c.closed}</span>
          </div>
        </div>
        <p className="text-charcoal-400 text-[10px] mt-4">
          {c.urgentNote}
        </p>
      </motion.div>
    </div>
  )
}
